...
class CreateTask extends React.Component {
  ...
  async componentDidMount() {
    const response = await this.props.apiClient.me.wallet.effectiveFunds()
    this.setState({
  ...
  async handleSubmit() {
    ...
    const response = await this.props.apiClient.tasks.create(this.state.form)
  ...
  handleChange = name => event => {
    ...
    if (name === 'amount' || name === 'reward') {
      const totalCost =
        event.target.value *
        (name === 'amount' ? this.state.form.reward : this.state.form.amount)
      if (totalCost > this.state.effectiveFunds)
        this.setState({ validCreation: true })
  ...
  render() {
    ...
    return (
      <div className={classes.pageContainer}>
        <Typography variant="title">Create a task</Typography>
        <Grid container spacing={24}>
          <Grid item xs={10}>
            <form className={classes.container} noValidate autoComplete="off">
              ...
              <Button
                disabled={this.state.validCreation}
                variant="outlined"
                color="primary"
...
export default connect(mapStateToProps)(withStyles(styles)(CreateTask))
