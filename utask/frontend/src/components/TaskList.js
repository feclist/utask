...
import { withStyles, Divider, Typography } from '../../node_modules/@material-ui/core';
...
class TaskList extends React.Component {
    ...
    async componentDidMount() {
        const taskResponse = await window.apiClient.tasks.userTasks()
        this.setState({
    ...
    render() {
        ...
        return (
            <div className={classes.root} style={this.props.style}>
                ...
                {
                    this.state.live_tasks.map((task) =>
                        task !== undefined &&
                        <TaskCard key={task.id} task={task} onDoTask={() => this.onDoTask(task.id)} loading={this.state.loadingTask === task.id} />
                    )
                }
                </div>
